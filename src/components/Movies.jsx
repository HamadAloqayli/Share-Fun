...
const Movies = () => {
    ...
    const shareWithGroup = (groupID,groupName,movie) => {
        ...
        firestore.collection('Groups').doc(groupID).collection('movies').where('id','==',movie.id).get()
        .then(res => {
            
            if(res.empty)
            {
                firestore.collection('Groups').doc(groupID).collection('movies').add({
                    id: movie.id,
                ...
                .then(res => {
                    hideGroups(movie);
                    tempShowMessage(`${movie.title} movie shared with ${groupName} group`,'suc');
                })
                .catch(res => {
            ...
            {
                tempShowMessage(`${movie.title} movie is already exists in ${groupName} group`,'err');
            }
            
        })
        .catch(res => {
            tempShowMessage('somethign went wrong!','err');
        });
    }
    ...
    const addToFavorite = (movie) => {
        ...
        if(user || sessionStorage.getItem('id').length > 0)
        {
            ...
            .then(res => {
                ...
                if(res.empty)
                {
                    firestore.collection('Users').doc(user.uid).collection('movies').add({
                        id: movie.id,
                        poster_path: movie.poster_path,
                        title: movie.title,
                        vote_average: movie.vote_average,
                ...
                {
                    tempShowMessage(`${movie.title} movie is already exists in favorite`,'err');;
                }
            })
            .catch(res => tempShowMessage('somethign went wrong!','err'));
        }
            ...
            <div className="parentAlert" style={(!errMessage)?{display:'none'}:{display:'block'}}>
                ...
                <div className="searchMovie my-4">
                    ...
                    <div className="input-group mb-2">
                        ...
                        <input type="text" ref={inputMovie} onChange={getMovie} className="form-control" id="inlineFormInputGroup" placeholder="Find your movie ..." />
                    </div>
                </div>
